...
try {
    ...
    // Setup: Get a test vehicle
    $stmt = $db->prepare("SELECT id FROM vehicles LIMIT 1");
    $stmt->execute();
    $vehicleId = $stmt->fetchColumn() ?: 1;
    ...
    echo "[Test 1] Booking vehicle $vehicleId...\n";
    try {
        $bookingId = $conflict->bookResource('Vehicle', $vehicleId, $startTime, $endTime, 'plans', 9999);
        echo "[PASS] Booking created: ID $bookingId\n\n";
    } catch (Exception $e) {
        echo "[INFO] Vehicle already booked or error: {$e->getMessage()}\n\n";
    }
    
    // ======= TEST 2: Attempt double-booking =======
    echo "[Test 2] Attempting double-booking same vehicle...\n";
    try {
        $conflict->bookResource('Vehicle', $vehicleId, $startTime, $endTime, 'plans', 8888);
    ...
    $hasConflict = $conflict->hasConflict('Vehicle', $vehicleId, $overlapStart, $overlapEnd);
    if ($hasConflict) {
    ...
    $hasConflict = $conflict->hasConflict('Vehicle', $vehicleId, $laterStart, $laterEnd);
    if (!$hasConflict) {
    ...
    $hasConflict = $conflict->hasConflict('Vehicle', $vehicleId, $startTime, $endTime);
    if (!$hasConflict) {
